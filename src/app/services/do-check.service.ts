...
export class DoCheckService {
  ...
  private startWatchAuthentication(): void {
    this.isRunning = true;
    this.printer.log(enums.busEnum.DO_CHECK_SERVICE, 'start watch: authentication');

    if (this.checkAuthSub === null) {

      this.checkAuthSub = observableInterval(2000)
        .subscribe(() => {
          this.printer.log(enums.busEnum.DO_CHECK_SERVICE, '----------CHECK------------');
          ...
          if (this.auth.authenticated()) {
            ...
            if (this.userId === undefined) {
              this.printer.log(enums.busEnum.DO_CHECK_SERVICE, 'user_id undefined, reloading location...');
            ...
            this.printer.log(enums.busEnum.DO_CHECK_SERVICE, 'tokenEmail:', tokenEmail);

            if (this.userId !== tokenEmail) {
              this.printer.log(enums.busEnum.DO_CHECK_SERVICE, 'user_id mismatch, reloading location...');
              ...
              this.printer.log(enums.busEnum.DO_CHECK_SERVICE, 'user_id match, complete');
            ...
            if ([
              ...
              'project',
              'project-deleted',
            ].indexOf(pathArray[0]) > -1) {

              this.printer.log(enums.busEnum.DO_CHECK_SERVICE, 'not authenticated,  dispatching Logout...');
  ...
  private startWatchWebSocketMessage(): void {
    this.printer.log(enums.busEnum.DO_CHECK_SERVICE, 'start watch: websocket message timestamp');

    this.lastTs = null;

    if (this.checkLastWsMsgTsSub === null) {

      this.checkLastWsMsgTsSub = this.store.select(selectors.getLayoutLastWsMsgTs)
        .pipe(filter(v => !!v))
        .subscribe(currentTs => {
          ...
          if (this.lastTs) {
            ...
            this.printer.log(enums.busEnum.DO_CHECK_SERVICE, 'ping timestamp difference:', `${diff}s`);

            if (diff > 600) { // TODO: change 10 min to 30 sec and message to 'connection lost'
  ...
  private stopWatchWebSocketMessage(): void {
    if (this.checkLastWsMsgTsSub) { this.checkLastWsMsgTsSub.unsubscribe(); }
