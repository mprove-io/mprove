  ...
  (
    ...
    if (model) {
      ...
      newNodes = model.nodes
        .map((nodeAs: api.ModelNode) => Object.assign({}, nodeAs, {
          children: nodeAs.children
            .map((nodeF: api.ModelNode) =>
              nodeF.is_field
                ? Object.assign({}, nodeF, { field: newFields.find(field => field.id === nodeF.id) })
                : Object.assign({}, nodeF, {
                  children: nodeF.children
                    .map((nodeS: api.ModelNode) =>
                      nodeS.is_field
                        ? Object.assign({}, nodeS, { field: newFields.find(field => field.id === nodeS.id) })
                        : nodeS
                    )
                }))
        }));
