...
export function projectsReducer(state = initialState, action: types.ProjectsActions): api.Project[] {
  ...
  switch (action.type) {
    ...
    case actionTypes.UPDATE_PROJECTS_STATE: {
      ...
      action.payload.forEach((cv) => {
        ...
        let index = newState.findIndex((project) => project.project_id === cv.project_id);
    ...
    case actionTypes.REMOVE_PROJECT: {
      ...
      let index = newState.findIndex(project => project.project_id === action.payload);
