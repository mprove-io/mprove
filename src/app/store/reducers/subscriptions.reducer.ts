...
export function subscriptionsReducer(state = initialState, action: types.SubscriptionsActions):
  api.Subscription[] {
  ...
  switch (action.type) {
    ...
    case actionTypes.UPDATE_SUBSCRIPTIONS_STATE: {
      ...
      action.payload.forEach((cv) => {
        let index = newState.findIndex((subscription) =>
          subscription.subscription_id === cv.subscription_id
