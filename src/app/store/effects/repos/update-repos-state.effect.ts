...
export class UpdateReposStateEffect {
  ...
  @Effect({ dispatch: false }) updateReposState$: Observable<Action> = this.actions$
    ...
    .pipe(
      tap((action: actions.UpdateReposStateAction) => {
        ...
        if (selectedRepo) {
          ...
          if (selectedModel && selectedModel.struct_id !== selectedRepo.struct_id) {

            this.printer.log(enums.busEnum.UPDATE_REPOS_EFFECT, 'selected model struct_id mismatch');
          ...
          } else if (selectedDashboard && selectedDashboard.struct_id !== selectedRepo.struct_id) {
