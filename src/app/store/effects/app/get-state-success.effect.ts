...
export class GetStateSuccessEffect {
  ...
  @Effect() getStateSuccess$: Observable<Action> =
    this.actions$.ofType(actionTypes.GET_STATE_SUCCESS)
      .pipe(
        tap((action: actions.GetStateSuccessAction) => Raven.setUserContext({
          email: action.payload.state.user.user_id,
        })),
        mergeMap((action: actions.GetStateSuccessAction) => from([
          new actions.UpdateProjectsStateAction(action.payload.state.projects),             // 1

          new actions.UpdateSubscriptionsStateAction(action.payload.state.subscriptions),   // 2
