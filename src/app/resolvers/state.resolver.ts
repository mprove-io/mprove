...
export class StateResolver implements Resolve<Observable<boolean>> {
  ...
  resolve(
    ...
    if (this.tokenEmail === undefined) {
      this.printer.log(
        enums.busEnum.STATE_RESOLVER,
        `JWT does not contain email, dispatching Logout...`
      );
    ...
    if (this.userId !== this.tokenEmail) {
      ...
      this.printer.log(
        enums.busEnum.STATE_RESOLVER,
        `authenticated, but user_id undefined or mismatch, getting State...`
      );
      ...
      this.printer.log(
        enums.busEnum.STATE_RESOLVER,
        `authenticated, user_id match, resolved (true)`
      );
      return of(true);
    }
  }
  ...
  getTokenEmail() {
    ...
    this.printer.log(
      enums.busEnum.STATE_RESOLVER_JWT,
      `decodeToken result:`,
