...
export class ModelResolver implements Resolve<boolean> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> {
    ...
    this.printer.log(enums.busEnum.MODEL_SELECTED_RESOLVER, 'modelId:', route.params['modelId']);
    ...
    if (!bqProject) {
      this.printer.log(enums.busEnum.MODEL_SELECTED_RESOLVER, `bqProject empty, navigating profile...`);
      this.router.navigate(['/profile']);

      this.myDialogService.showBqDialog();
      this.printer.log(enums.busEnum.MODEL_SELECTED_RESOLVER, `resolved (false)`);
      return of(false);
    }
    ...
    if (!selectedProjectModeRepoId) {
      this.printer.log(enums.busEnum.MODEL_SELECTED_RESOLVER, `selectedProjectModeRepoId undefined`);
      this.printer.log(enums.busEnum.MODEL_SELECTED_RESOLVER, `resolved (false)`);
      return of(false);
    }
  ...
  hasModelInStore(id: string): Observable<boolean> {
    return this.store.select(selectors.getModelsState)
      .pipe(
        mergeMap(models => {
          ...
          let exists = models.findIndex(model =>
            model.project_id === selectedProjectId &&
            model.repo_id === selectedProjectModeRepoId &&
            model.model_id === id &&
            model.struct_id === selectedProjectModeRepoStructId) > -1;

          if (!exists) {
            this.printer.log(enums.busEnum.MODEL_SELECTED_RESOLVER, `model not exists, navigating 404...`);

            this.store.select(selectors.getLayoutModelId)
              .pipe(take(1))
              .subscribe(x => x ? this.store.dispatch(new actions.UpdateLayoutModelIdAction(undefined)) : 0);
            ...
            this.printer.log(enums.busEnum.MODEL_SELECTED_RESOLVER, `model exists`);
            ...
            if (id !== layoutModelId) {
              this.printer.log(enums.busEnum.MODEL_SELECTED_RESOLVER, `selecting model...`);
              this.store.dispatch(new actions.UpdateLayoutModelIdAction(id));

              return this.store.select(selectors.getLayoutModelId)
                .pipe(
                  filter(selectedModelId => selectedModelId === id),
                  map(() => true),
                  tap(x => {
                    this.printer.log(enums.busEnum.MODEL_SELECTED_RESOLVER, `model selected`);
                    this.printer.log(enums.busEnum.MODEL_SELECTED_RESOLVER, `resolved (true)`);
                  }),
                  take(1)
                );
              ...
              this.printer.log(enums.busEnum.MODEL_SELECTED_RESOLVER, `model already selected`);
              this.printer.log(enums.busEnum.MODEL_SELECTED_RESOLVER, `resolved (true)`);
              return of(true);
            }
          }
