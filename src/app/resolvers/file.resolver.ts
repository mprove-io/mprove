...
export class FileResolver implements Resolve<boolean> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> {
    ...
    this.printer.log(enums.busEnum.FILE_SELECTED_RESOLVER, 'fileId:', route.params['fileId']);
    ...
    if (!selectedProjectModeRepoId) {
      this.printer.log(enums.busEnum.FILE_SELECTED_RESOLVER, `selectedProjectModeRepoId undefined`);
      this.printer.log(enums.busEnum.FILE_SELECTED_RESOLVER, `resolved (false)`);
      return of(false);
    }
  ...
  hasFileInStore(id: string): Observable<boolean> {
    return this.store.select(selectors.getFilesState)
      .pipe(
        mergeMap(files => {
          ...
          let exists = files.findIndex(file =>
            file.project_id === selectedProjectId &&
            file.repo_id === selectedProjectModeRepoId &&
            file.file_id === id &&
            file.deleted === false) > -1;

          if (!exists) {
            this.printer.log(enums.busEnum.FILE_SELECTED_RESOLVER, `file not exists, navigating 404...`);

            this.store.select(selectors.getLayoutFileId)
              .pipe(take(1))
              .subscribe(
                x => x ? this.store.dispatch(new actions.UpdateLayoutFileIdAction(undefined)) : 0);
            ...
            if (id !== layoutFileId) {
              this.printer.log(enums.busEnum.FILE_SELECTED_RESOLVER, `selecting file...`);
              this.store.dispatch(new actions.UpdateLayoutFileIdAction(id));

              return this.store.select(selectors.getLayoutFileId)
                .pipe(
                  filter(selectedFileId => selectedFileId === id),
                  map(() => true),
                  tap(x => {
                    this.printer.log(enums.busEnum.FILE_SELECTED_RESOLVER, `file selected`);
                    this.printer.log(enums.busEnum.FILE_SELECTED_RESOLVER, `resolved (true)`);
                  }),
                  take(1)
                );
              ...
              this.printer.log(enums.busEnum.FILE_SELECTED_RESOLVER, `file already selected`);
              this.printer.log(enums.busEnum.FILE_SELECTED_RESOLVER, `resolved (true)`);
              return of(true);
            }
          }
