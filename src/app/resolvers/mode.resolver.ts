...
export class ModeResolver implements Resolve<boolean> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<boolean> {
    ...
    if (!selectedProjectId) {
      this.printer.log(enums.busEnum.MODE_RESOLVER, `selectedProjectId undefined`);
    ...
    switch (route.params['mode']) {
      ...
      case enums.LayoutModeEnum.Prod: {
        ...
        this.printer.log(enums.busEnum.MODE_RESOLVER, 'will be resolved (true)');
        return this.store.select(selectors.getLayoutModeIsDev)
          .pipe(
            filter(isDev => !isDev),
            take(1)
          );
      ...
      case enums.LayoutModeEnum.Dev: {
        ...
        this.store.select(selectors.getSelectedProjectUserIsEditor)
          .pipe(take(1))
          .subscribe(x => userIsEditor = x);
        this.printer.log(enums.busEnum.MODE_RESOLVER, 'user is_editor:', userIsEditor);
