...
export class GenerateBlockmlDialogComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.reportYaml = y.safeDump({ reports: [rep] })
      .split('\n')
      .map(s => s
        .replace(/^\s\s/g, '')
        .replace(/^\s\s\s\s[-]/g, '  -')
  ...
  prepareReport() {
    ...
    if (filters && filters.length > 0) {
      ...
      filters.forEach((x: api.Filter) => {
        ...
        defaultFilters = Object.assign(defaultFilters, { [x.field_id]: bricks });
      });
  ...
  prepareOptions(chart: api.Chart) {
    ...
    if (rangeFillOpacity
      ...
      options = Object.assign(options, { range_fill_opacity: rangeFillOpacity });
    }
  ...
  prepareTile(chart: api.Chart) {
    ...
    if (tileWidth &&
      tileWidth !== api.ChartTileWidthEnum._6) {
    ...
    if (tileHeight &&
      tileHeight !== api.ChartTileHeightEnum._500) {
    ...
    if (viewSize &&
      viewSize !== api.ChartViewSizeEnum.Auto) {
