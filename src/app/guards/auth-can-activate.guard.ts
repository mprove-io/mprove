...
export class AuthCanActivateGuard implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot, routerStateSnapshot: RouterStateSnapshot): boolean {
    ...
    if (this.auth.authenticated()) {
      this.printer.log(enums.busEnum.AUTH_CAN_ACTIVATE, 'authenticated, resolved (true)');
      return true;
      ...
      this.printer.log(enums.busEnum.AUTH_CAN_ACTIVATE, 'setting redirect_url:', url);
      localStorage.setItem('redirect_url', url);

      this.printer.log(enums.busEnum.AUTH_CAN_ACTIVATE, 'navigating login...', url);
