...
export class UpdateProjectTimezoneComponent implements OnChanges {
  ...
  ngOnChanges() {
    ...
    } else if (this.updateProjectTimezoneForm.get('projectTimezone').value === this.selectedProject.timezone) {
  ...
  onSubmit(form: FormGroup, fv: any) {
    this.store.dispatch(new actions.SetProjectTimezoneAction({
