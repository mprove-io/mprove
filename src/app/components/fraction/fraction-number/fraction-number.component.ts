...
export class FractionNumberComponent implements OnInit {
  ...
  getValueBrick(fractionType: api.FractionTypeEnum, value: string) {
    let newBrick =
      fractionType === api.FractionTypeEnum.NumberIsEqualTo ? value :
        fractionType === api.FractionTypeEnum.NumberIsNotEqualTo ? `not ${value}` : '';
  ...
  getSingleBrick(fractionType: api.FractionTypeEnum, value: number) {
    let newBrick =
      fractionType === api.FractionTypeEnum.NumberIsGreaterThan ? `> ${value}` :
        fractionType === api.FractionTypeEnum.NumberIsGreaterThanOrEqualTo ? `>= ${value}` :
          fractionType === api.FractionTypeEnum.NumberIsLessThan ? `< ${value}` :
            fractionType === api.FractionTypeEnum.NumberIsLessThanOrEqualTo ? `<= ${value}` : '';
  ...
  getBetweenBrick(
    ...
    let content =
      option === api.FractionNumberBetweenOptionEnum.Inclusive ? `[${n1}, ${n2}]` :
        option === api.FractionNumberBetweenOptionEnum.LeftInclusive ? `[${n1}, ${n2})` :
          option === api.FractionNumberBetweenOptionEnum.RightInclusive ? `(${n1}, ${n2}]` :
            option === api.FractionNumberBetweenOptionEnum.Exclusive ? `(${n1}, ${n2})` : '';

    let newBrick = fractionType === api.FractionTypeEnum.NumberIsBetween ? content :
      fractionType === api.FractionTypeEnum.NumberIsNotBetween ? `not ${content}` : '';
  ...
  typeChange(ev: MatSelectChange) {
    switch (ev.value) {
      ...
      case (this.fractionTypeEnum.NumberIsBetween): {
        let newBetweenOption = this.fraction.number_between_option || api.FractionNumberBetweenOptionEnum.Inclusive;

        let newBrick = this.getBetweenBrick(
          ev.value, newBetweenOption, this.fraction.number_value1, this.fraction.number_value2);
  ...
  betweenOptionChange(ev: MatSelectChange) {
    ...
    let newBrick = this.getBetweenBrick(
      this.fraction.type, newBetweenOption, this.fraction.number_value1, this.fraction.number_value2);
  ...
  numberValuesBlur(numberValues: FormControl) {
    if (numberValues.value !== this.fraction.number_values) {
      ...
      let newBrick = this.getValueBrick(
        this.fraction.type,
        numberValues.value,
      );
