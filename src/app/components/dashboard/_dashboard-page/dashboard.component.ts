...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  lastRunBy$ = this.store.select(selectors.getQueriesState)
    .pipe(
      filter(v => !!v),
      map(queries => {

        let reports: api.Report[] = [];
        this.store.select(selectors.getSelectedProjectModeRepoDashboardReports)
          .pipe(take(1))
  ...
  inProgressQueryExists$ = this.store.select(selectors.getQueriesState)
    .pipe(
      filter(v => !!v),
        ...
        if (reports.length > 0) {
          reports.forEach(
            report => {
              ...
              if (query && (query.status === api.QueryStatusEnum.Waiting
  ...
  allQueriesHaveData$ = this.store.select(selectors.getQueriesState)
    .pipe(
      filter(v => !!v),
      map((queries: api.Query[]) => {
  ...
  reports: api.Report[] = [];
  reports$ = this.store.select(selectors.getSelectedProjectModeRepoDashboardReports)
    .pipe(
      map(x => this.reports = x ? x : []) // no filter
    );

  dashboardId: string;
  dashboardId$ = this.store.select(selectors.getSelectedProjectModeRepoDashboardId)
    .pipe(
      filter(v => !!v),
      tap(x => {
        this.dashboardId = x;
  ...
  dashboardIsTemp$ = this.store.select(selectors.getSelectedProjectModeRepoDashboardIsTemp)
    .pipe(
      tap(x => {
  ...
  dashboardTitle$ = this.store.select(selectors.getSelectedProjectModeRepoDashboardTitle)
    .pipe(
      tap(x => {
  ...
  dashboardDescription$ = this.store.select(selectors.getSelectedProjectModeRepoDashboardDescription);
  ...
  visuals: any[] = [];
  visuals$ = this.store.select(selectors.getSelectedProjectModeRepoDashboardVisuals)
    .pipe(
      filter(v => !!v),
  ...
  fileId$ = this.store.select(selectors.getSelectedProjectModeRepoDashboardFileId)
    .pipe(
  ...
  cancel() {
    ...
    if (queryIds.length > 0) {
      this.store.dispatch(new actions.CancelQueriesAction({ query_ids: queryIds }));
    }
  }
  ...
  run() {
    ...
    if (queryIds.length > 0) {
      this.store.dispatch(new actions.RunQueriesAction({ query_ids: queryIds, refresh: false }));
    }
  }
  ...
  runRefresh() {
    ...
    if (queryIds.length > 0) {
      this.store.dispatch(new actions.RunQueriesAction({ query_ids: queryIds, refresh: true }));
    }
  }
