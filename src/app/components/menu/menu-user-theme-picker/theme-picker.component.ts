...
export class ThemePickerComponent {
  ...
  mainTheme$ = this.store
    .select(selectors.getSelectedProjectUserMainTheme)
    .pipe(
      filter(v => !!v),
      tap(mainTheme => {
        if (this.mainTheme) {
  ...
  sqlTheme$ = this.store.select(selectors.getSelectedProjectUserSqlTheme).pipe(
    filter(v => !!v),
    tap(sqlTheme => {
  ...
  userMember$ = this.store.select(selectors.getSelectedProjectUserMember).pipe(
    filter(v => !!v),
    tap(x => {
      this.userMember = x;
    })
  ...
  installMainTheme(mainTheme: api.MemberMainThemeEnum) {
    this.store.dispatch(
      new actions.EditMemberAction(<any>(
        Object.assign({}, this.userMember, { main_theme: mainTheme })
      ))
    );
  }
  ...
  installDashTheme(dashTheme: api.MemberDashThemeEnum) {
    this.store.dispatch(
      new actions.EditMemberAction(<any>(
        Object.assign({}, this.userMember, { dash_theme: dashTheme })
      ))
    );
  }
  ...
  installFileTheme(fileTheme: api.MemberFileThemeEnum) {
    this.store.dispatch(
      new actions.EditMemberAction(<any>(
        Object.assign({}, this.userMember, { file_theme: fileTheme })
      ))
    );
  }
  ...
  installSqlTheme(sqlTheme: api.MemberSqlThemeEnum) {
    this.store.dispatch(
      new actions.EditMemberAction(<any>(
        Object.assign({}, this.userMember, { sql_theme: sqlTheme })
      ))
    );
  }
}
