...
export async function createMconfigAndQuery(req: Request, res: Response) {
  ...
  let payload: api.CreateMconfigAndQueryRequestBodyPayload = validator.getPayload(req);
  ...
  let [project, repo, model] =
    <[entities.ProjectEntity, entities.RepoEntity, entities.ModelEntity]>await Promise.all([
  ...
  if (!model) { throw new ServerError({ name: enums.otherErrorsEnum.MODEL_NOT_FOUND }); }

  let itemProcessQuery = <interfaces.ItemProcessQuery>await blockml.processQuery({
    ...
    .catch(e => helper.reThrow(e, enums.blockmlErrorsEnum.BLOCKML_PROCESS_QUERY));
