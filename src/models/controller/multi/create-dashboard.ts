...
export async function createDashboard(req: Request, res: Response) {
  ...
  let payload: api.CreateDashboardRequestBodyPayload = validator.getPayload(req);
  ...
  let [project, repo, oldDashboard, models] =
    <[entities.ProjectEntity, entities.RepoEntity, entities.DashboardEntity, entities.ModelEntity[]]>
    await Promise.all([
      ...
      storeModels.find({
        ...
        .catch(e => helper.reThrow(e, enums.storeErrorsEnum.STORE_MODELS_FIND_ONE)),
    ])
  ...
  if (!oldDashboard) { throw new ServerError({ name: enums.otherErrorsEnum.DASHBOARD_NOT_FOUND }); }
  ...
  let itemProcessDashboard = <interfaces.ItemProcessDashboard>await blockml.processDashboard({
    project_id: projectId,
    repo_id: repoId,
    ...
    .catch(e => helper.reThrow(e, enums.blockmlErrorsEnum.BLOCKML_PROCESS_DASHBOARD));

  // update server_ts
  let newServerTs = helper.makeTs();

  itemProcessDashboard.mconfigs = helper.refreshServerTs(itemProcessDashboard.mconfigs, newServerTs);
  itemProcessDashboard.queries = helper.refreshServerTs(itemProcessDashboard.queries, newServerTs);
  ...
  let responsePayload: api.CreateDashboardResponse200BodyPayload = {
    ...
    dashboard_mconfigs: itemProcessDashboard.mconfigs.map(
      mconfig => wrapper.wrapToApiMconfig(mconfig)),
    dashboard_queries: itemProcessDashboard.queries.map(
