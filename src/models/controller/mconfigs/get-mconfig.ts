...
export async function getMconfig(req: Request, res: Response) {
  ...
  let mconfig = await storeMconfigs.findOne({ mconfig_id: mconfigId })
    .catch(e => helper.reThrow(e, enums.storeErrorsEnum.STORE_MCONFIGS_FIND_ONE));

  if (!mconfig) { throw new ServerError({ name: enums.otherErrorsEnum.MCONFIG_NOT_FOUND }); }
