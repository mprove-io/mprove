...
export async function runQueriesDry(req: Request, res: Response) {
  ...
  let project = <entities.ProjectEntity>await storeProjects.findOne({
  ...
  let results = <interfaces.ItemRunQueryDry[]>await Promise.all(
    queries.map(async query => <Promise<interfaces.ItemRunQueryDry>>
      proc.runQueryDry({
        query: query,
        new_last_run_dry_ts: newLastRunDryTs,
        credentials_file_path: project.bigquery_credentials_file_path
      })
        .catch(e => helper.reThrow(e, enums.procErrorsEnum.PROC_RUN_QUERY_DRY))
    )
  )
    .catch(e => helper.reThrow(e, enums.storeErrorsEnum.STORE_PROJECTS_FIND_ONE));

  let validEstimates = results.filter(result => !!result.valid_estimate)
    .map(x => x.valid_estimate);

  let errorQueries = results.filter(result => !!result.error_query)
    .map(x => x.error_query);
