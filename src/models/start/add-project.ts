...
export async function addProject(item: {
  ...
  let dbProject = await storeProjects.findOne(projectId)
  ...
  let users = <entities.UserEntity[]>await storeUsers.find({
    user_id: In([config.ADMIN_1_USER_ID])
  })
    .catch(e => helper.reThrow(e, enums.storeErrorsEnum.STORE_USERS_FIND));

  let members: entities.MemberEntity[] = users.map(user => generator.makeMember({
    user: user,
    project_id: projectId,
  ...
  let isCentralExistOnDisk = <boolean>await disk.isRepoExistOnDisk({
    project_id: projectId,
    repo_id: constants.CENTRAL_REPO_ID
  })
    .catch(e => helper.reThrow(e, enums.diskErrorsEnum.DISK_IS_REPO_EXIST_ON_DISK));

  let isProdExistOnDisk = <boolean>await disk.isRepoExistOnDisk({
    project_id: projectId,
  ...
  let {
    ...
    models: prodModels,
    queries: prodQueries } = itemStruct;
  ...
  await forEach(memberIds, async (repoId: string) => {
    ...
    let itemCatalogRepo = <interfaces.ItemCatalog>await disk.getRepoCatalogNodesAndFiles({
      project_id: projectId,
      repo_id: repoId,
    });
    ...
    let {
      ...
      models: repoModels,
      errors: repoErrors,
      mconfigs: repoMconfigs } = structCopyItem;
