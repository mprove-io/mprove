        ...
        if (field.from_field.match(ApRegex.WORD_CHARACTERS())) {
          ...
          if (!fromField) {
            ...
            ErrorsCollector.addError(new AmError({
              ...
              message: `field "${field.from_field}" is missing or not valid in fields section of "${x.name}" model`,
              lines: [{
          ...
          if (r) {
            ...
            switch (true) {
              ...
              case asName !== 'mf': {
                ...
                if (!join) {
                  ...
                  ErrorsCollector.addError(new AmError({
                    title: `missing alias`,
                    message: `alias "${asName}" is missing in joins section of "${x.name}" model`,
                    lines: [{
                      line: field.from_field_line_num,
                      name: x.file,
                      path: x.path,
                  ...
                  let viewField = join.view.fields.find(vf => vf.name === viewFieldName);

                  if (!viewField) {
                    // error e232
                    ErrorsCollector.addError(new AmError({
                      title: `missing view field`,
                      message: `field '${viewFieldName}' is missing or not valid in fields section` +
                        ` of '${join.view.name}' view with "${asName}" alias in "${x.name}" model`,
                      lines: [{
                        line: field.from_field_line_num,
                        name: x.file,
                        path: x.path,
            ...
            ErrorsCollector.addError(new AmError({
              ...
              message: `model filter "from_field:" value must be specified in ` +
