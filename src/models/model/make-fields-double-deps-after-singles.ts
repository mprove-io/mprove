    ...
    x.fields.forEach(f => {
      ...
      while (restart) {
        ...
        while ((r = reg.exec(f.sql_real))) {
          ...
          if (depField.field_class === enums.FieldClassEnum.Calculation) {
            f.sql_real = ApRegex.replaceAndConvert(
              f.sql_real,
              depField.sql_real,
              asName,
              depName
            ...
            x.fields_double_deps_after_singles[f.name][asName][depName] =
              f.sql_line_num;

            if (
              f.field_class === enums.FieldClassEnum.Calculation &&
      ...
      if (
        ...
        while ((r2 = reg2.exec(f.sql_key_real))) {
          ...
          x.fields_double_deps_after_singles[f.name][asName2][depName2] =
            f.sql_key_line_num;
        }
      }
    });
