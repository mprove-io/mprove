...
export function wrapStructResponse(struct: interfaces.ItemStructAndRepo): api.Struct {
  return {
    errors: struct.errors.map(error => wrapToApiError(error)),
    models: struct.models.map(model => wrapToApiModel(model)),
    dashboards: struct.dashboards.map(dashboard => wrapToApiDashboard(dashboard)),
