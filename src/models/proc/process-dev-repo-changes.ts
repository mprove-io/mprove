...
export async function processDevRepoChanges(item: {
  ...
  filesItem.new_files = helper.refreshServerTs(filesItem.new_files, newServerTs);
  filesItem.changed_files = helper.refreshServerTs(filesItem.changed_files, newServerTs);
  filesItem.deleted_files = helper.refreshServerTs(filesItem.deleted_files, newServerTs);
  ...
  await connection.transaction(async manager => {
    ...
    if (filesItem.deleted_files.length > 0) { // TODO: use custom transaction to not check length

      let deletedFilesIds = filesItem.deleted_files.map(file => file.file_absolute_id);

      let storeFilesTrans = store.getFilesRepo(manager);

      await storeFilesTrans.delete(deletedFilesIds)
