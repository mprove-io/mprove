...
export async function checkRunningQuery(item: {
  ...
  if (query.is_checking === enums.bEnum.TRUE) {
    ...
    await storeQueries.save(query)
      .catch(e => helper.reThrow(e, enums.storeErrorsEnum.STORE_QUERIES_SAVE));
  }


  let credentialsFilePath = `${config.DISK_BIGQUERY_CREDENTIALS_PATH}/${query.project_id}.json`;
  ...
  if (query.bigquery_is_copying === enums.bEnum.TRUE) {
    ...
    query = <entities.QueryEntity>await checkCopyingQuery({
      ...
      .catch(e => helper.reThrow(e, enums.procErrorsEnum.PROC_CHECK_COPYING_QUERY));
    ...
    query = <entities.QueryEntity>await checkNotCopyingQuery({
      ...
      .catch(e => helper.reThrow(e, enums.procErrorsEnum.PROC_CHECK_NOT_COPYING_QUERY));
  }
...
async function checkCopyingQuery(item: {
  ...
  let itemCopyJob = await bigqueryCopyJob.get()
    .catch((e: any) => helper.reThrow(e, enums.bigqueryErrorsEnum.BIGQUERY_JOB_GET));
  ...
  if (copyJobGetResponse.status.state === 'DONE') {
    ...
    if (copyJobGetResponse.status.errorResult) {
      ...
      let itemTableDelete = await bigqueryTable.delete()
        .catch((e: any) => helper.reThrow(e, enums.bigqueryErrorsEnum.BIGQUERY_TABLE_DELETE));
    }
  }
...
async function checkNotCopyingQuery(item: {
  ...
  if (queryJobGetResponse.status.state === 'DONE') {
    ...
    if (queryJobGetResponse.status.errorResult) {
      ...
      if (query.is_pdt === enums.bEnum.TRUE) { // bigquery_is_copying false

        let copyJobId = await copyQueryResultsToPdt({
          query: query,
          bigquery: bigquery
        })
          .catch((e: any) => helper.reThrow(e, enums.procErrorsEnum.PROC_COPY_QUERY_RESULTS_TO_PDT));
        ...
        let queryResultsItem = await queryJob.getQueryResults()
          .catch((e: any) => helper.reThrow(e, enums.bigqueryErrorsEnum.BIGQUERY_JOB_GET_QUERY_RESULTS));
...
async function copyQueryResultsToPdt(item: {
  ...
  let itemPdtTableExists = await pdtTable.exists()
    .catch((e: any) => helper.reThrow(e, enums.bigqueryErrorsEnum.BIGQUERY_TABLE_EXISTS_CHECK));

  let pdtTableExists: boolean = itemPdtTableExists[0];

  if (pdtTableExists === true) {

    let itemPdtTableDelete = await pdtTable.delete()
      .catch((e: any) => helper.reThrow(e, enums.bigqueryErrorsEnum.BIGQUERY_TABLE_DELETE));
  }
  ...
  let itemTableCreateCopyJob = <any>await bigqueryTable.createCopyJob(pdtTable, {
    createDisposition: 'CREATE_IF_NEEDED',
    writeDisposition: 'WRITE_EMPTY'
  })
    .catch((e: any) => helper.reThrow(e, enums.bigqueryErrorsEnum.BIGQUERY_CREATE_COPY_JOB));
