...
export function checkFieldUnknownParameters
  ...
  item.entities.forEach(x => {
    ...
    x.fields.forEach(field => { // must be explicit

      Object.keys(field)
        .filter(k => !k.match(ApRegex.ENDS_WITH_LINE_NUM()) && ['name', 'field_class'].indexOf(k) < 0)
        .forEach(parameter => {

          if (parameter === 'hidden' && !field[parameter].match(ApRegex.TRUE_FALSE())) {
            // error e115
          ...
          if (Array.isArray((<any>field)[parameter]) &&
          ...
          } else if (!!(<any>field)[parameter] && (<any>field)[parameter].constructor === Object) {
            // error e114
            ErrorsCollector.addError(new AmError({
