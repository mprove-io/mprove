...
export function makeNeedsDoubles(item: interfaces.Vars) {
  ...
  Object.keys(item.model.sql_always_where_double_deps_after_singles).forEach(as => {
    Object.keys(item.model.sql_always_where_double_deps_after_singles[as]).forEach(dep => {
      let f = `${as}.${dep}`;
      whereDoubleDeps[f] = 1;
    });
  });
  ...
  Object.keys(item.model.sql_always_where_calc_double_deps_after_substitutions).forEach(as => {
    Object.keys(item.model.sql_always_where_calc_double_deps_after_substitutions[as]).forEach(dep => {
      let f = `${as}.${dep}`;
  ...
  Object.keys(item.model.sql_always_where_calc_deps_after_singles).forEach(dep => {
    let f = `mf.${dep}`;
    whereCalcDeps[f] = 1;
  });
  ...
  elements.forEach(element => {
    ...
    if (!needsDoubles[aName]) { needsDoubles[aName] = {}; }
  ...
  if (needsDoubles['mf']) {
    ...
    Object.keys(needsDoubles['mf']).forEach(fieldName => {
      Object.keys(item.model.fields_deps_after_singles[fieldName]).forEach(dep => {
    ...
    Object.keys(needsDoubles['mf']).forEach(fieldName => {
      Object.keys(item.model.fields_double_deps_after_singles[fieldName]).forEach(alias => {
        Object.keys(item.model.fields_double_deps_after_singles[fieldName][alias]).forEach(dep => {
          if (!needsDoubles[alias]) { needsDoubles[alias] = {}; }
  ...
  Object.keys(needsDoubles).filter(a => a !== 'mf').forEach(asName => {
    Object.keys(needsDoubles[asName]).forEach(fieldName => {

      let join = item.model.joins.find(j => j.as === asName);

      Object.keys(join.view.fields_deps_after_singles[fieldName]).forEach(dep => {
