...
export function composeMain(item: interfaces.Vars) {
  ...
  if (item.joins_where.length > 0 ||
    whereMainLength > 0 ||
    (typeof item.model.sql_always_where_real !== 'undefined' && item.model.sql_always_where_real !== null)) {
    ...
    if (typeof item.model.sql_always_where_real !== 'undefined' && item.model.sql_always_where_real !== null) {
      // remove ${ } on doubles (no singles exists in _real of sql_always_where)
      // ${a.city} + ${b.country}   >>>   a.city + b.country

      let sqlAlwaysWhereFinal = ApRegex.removeBracketsOnDoubles(item.model.sql_always_where_real);
