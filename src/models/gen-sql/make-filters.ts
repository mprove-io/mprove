...
export function makeFilters(item: interfaces.Vars) {
  ...
  Object.keys(item.model.filters).forEach(modelFilter => {
    ...
    if (typeof item.filters[modelFilterName] === 'undefined' || item.filters[modelFilterName] === null) {
  ...
  item.model.joins_sorted.forEach(asName => {
    ...
    Object.keys(join.view.filters).forEach(viewFilter => {
      ...
      if (typeof item.filters[viewFilterName] === 'undefined' || item.filters[viewFilterName] === null) {
  ...
  [
    ...
    let field = asName === 'mf'
      ...
      : item.model.joins.find(j => j.as === asName).view.fields.find(vField => vField.name === fieldName);
    ...
    if (field.result === enums.FieldExtResultEnum.Ts) {
      ...
      if (field.field_class === enums.FieldClassEnum.Filter) {
        ...
        sqlTimestampSelect = ApRegex.removeBracketsOnDoubles(field.sql_timestamp_real);
    ...
    let proc = field.field_class === enums.FieldClassEnum.Measure
      ? `${asName}_${fieldName}`
    ...
    // aa
    if (myORs.length === 0 && myNOTs.length === 0) {       //  not interested
