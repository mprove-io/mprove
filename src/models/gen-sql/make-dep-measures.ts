    ...
    [...item.select, ...Object.keys(item.filters)].forEach(element => {
      ...
      if (asName === 'mf') {
        ...
        if (field.field_class === enums.FieldClassEnum.Calculation) {
          Object.keys(item.model.fields_deps_after_singles[fieldName]).forEach(depName => {

            let depModelField = item.model.fields.find(mField => mField.name === depName);
        ...
        Object.keys(item.model.fields_double_deps_after_singles[fieldName]).forEach(alias => {
          Object.keys(item.model.fields_double_deps_after_singles[fieldName][alias]).forEach(depName => {

            let join = item.model.joins.find(j => j.as === alias);

            let depViewField = join.view.fields.find(vField => vField.name === depName);
        ...
        if (field.field_class === enums.FieldClassEnum.Calculation) {
          Object.keys(join.view.fields_deps_after_singles[fieldName]).forEach(depName => {
            let depViewField = join.view.fields.find(vField => vField.name === depName);
      ...
      if (item.model.sql_always_where_calc_deps_after_singles) {
        Object.keys(item.model.sql_always_where_calc_deps_after_singles).forEach(depName => {
          let depModelField = item.model.fields.find(mField => mField.name === depName);
      ...
      if (item.model.sql_always_where_calc_double_deps_after_substitutions) {
        Object.keys(item.model.sql_always_where_calc_double_deps_after_substitutions).forEach(alias => {
          Object.keys(item.model.sql_always_where_calc_double_deps_after_substitutions[alias]).forEach(depName => {
            let join = item.model.joins.find(j => j.as === alias);
