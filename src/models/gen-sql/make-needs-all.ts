...
export function makeNeedsAll(item: interfaces.Vars) {
  ...
  Object.keys(item.joins)
    ...
    .forEach(alias => {
      Object.keys(item.model.joins_double_deps_after_singles[alias]).forEach(asName => {
        Object.keys(item.model.joins_double_deps_after_singles[alias][asName]).forEach(dep => {
          if (!needsAll[asName]) { needsAll[asName] = {}; }
