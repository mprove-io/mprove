    ...
    x.reports.forEach(report => {
      Object.keys(report)
        ...
        .forEach(parameter => {
          ...
          if (Array.isArray((<any>report)[parameter]) && ['select'].indexOf(parameter) < 0) {
            // error e134
            ErrorsCollector.addError(new AmError({
          ...
          if ((!!(<any>report)[parameter] && (<any>report)[parameter].constructor === Object) &&
            ['default_filters', 'listen_filters', 'data', 'axis', 'options', 'tile'].indexOf(parameter) < 0) {
            // error e135
            ErrorsCollector.addError(new AmError({
