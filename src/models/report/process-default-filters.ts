            ...
            if (!modelField) {
              ...
              ErrorsCollector.addError(
                new AmError({
                  ...
                  message:
                    `found default filter "${defaultFilter}" references field "${fieldName}" that is missing ` +
            ...
            if (!join) {
              ...
              ErrorsCollector.addError(
                new AmError({
                  title: `wrong alias`,
                  message:
                    `found default filter "${defaultFilter}" references missing alias ` +
            ...
            if (!viewField) {
              ...
              ErrorsCollector.addError(
                new AmError({
                  ...
                  message:
                    `found default filter "${defaultFilter}" references field "${fieldName}" that is missing ` +
                    `or not valid in fields section of view "${
                      join.view.name
                    }" with "${asName}" alias ` +
                    `in "${model.name}" model`,
                  lines: [
