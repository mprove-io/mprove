...
export async function pushToCentral(item: {
  ...
  let repoPath = `${config.DISK_BASE_PATH}/${item.project_id}/${item.from_repo_id}`;
  ...
  let originRemote = <nodegit.Remote>await gitRepo.getRemote('origin')
    .catch(e => helper.reThrow(e, enums.nodegitErrorsEnum.NODEGIT_REPO_GET_REMOTE));

  await originRemote.push(['refs/heads/master:refs/heads/master'], constantFetchOptions)
