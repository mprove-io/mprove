...
export async function prepareCentralAndProd(projectId: string) {
  let centralDir = `${config.DISK_BASE_PATH}/${projectId}/${
    constants.CENTRAL_REPO_ID
  }`;
  let prodDir = `${config.DISK_BASE_PATH}/${projectId}/${
  ...
  await initCentralRepo(projectId).catch(e =>
    helper.reThrow(e, enums.gitErrorsEnum.GIT_INIT_CENTRAL_REPO)
  );

  await cloneCentralToProd(projectId).catch(e =>
    helper.reThrow(e, enums.gitErrorsEnum.GIT_CLONE_CENTRAL_TO_PROD)
  );

  await createInitialCommitToProd(projectId).catch(e =>
