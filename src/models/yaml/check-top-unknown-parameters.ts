...
export function checkTopUnknownParameters(item: {
  ...
  item.filesAny.forEach(file => {
    ...
    Object.keys(file)
      ...
      .forEach(parameter => {
        if (['path', 'ext', 'name'].indexOf(parameter) > -1) { return; }
        ...
        if (Array.isArray(file[parameter]) &&
          ['udfs', 'fields', 'reports', 'joins', 'access_users'].indexOf(parameter) < 0) {
          // error e120
          ErrorsCollector.addError(new AmError({
        ...
        } else if (!!file[parameter] && file[parameter].constructor === Object) {
          // error e121
          ErrorsCollector.addError(new AmError({
