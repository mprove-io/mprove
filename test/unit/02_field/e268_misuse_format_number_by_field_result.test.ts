...
jest.setTimeout(30000); test('testName', () => {
  ...
  return ApStruct.rebuildStruct({
    ...
    expect(struct.errors[0].title).toEqual(`misuse of format_number by field's result`);
    expect(struct.errors[0].message).toEqual(`format_number can only be used with fields where result is "number". ` +
