...
// yarn jest test/unit/04_model/e15_too_many_from_view.test.ts
jest.setTimeout(30000); test('testName', () => {
  expect.assertions(3);

  return ApStruct.rebuildStruct({
    dir: 'test/unit/04_model/e15',
    ...
    expect(struct.errors[0].message).toEqual(`model must have only one Join with 'from_view' parameter`);
  });
});
