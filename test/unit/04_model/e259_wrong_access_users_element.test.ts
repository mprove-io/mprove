...
// yarn jest test/unit/04_model/e259_wrong_access_users_element.test.ts
jest.setTimeout(30000); test('testName', () => {
  expect.assertions(3);

  return ApStruct.rebuildStruct({
    dir: 'test/unit/04_model/e259',
    ...
    expect(struct.errors[0].message).toEqual(`found array element that is not a single value`);
  });
});
