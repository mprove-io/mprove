...
// yarn jest test/valid/03_mea_refs_dim/v4_view_mea_refs_own_dim.test.ts
...
test('testName', () => {
  ...
  ((SELECT SUM(CAST(REGEXP_EXTRACT(val, \'\\\\|\\\\|(\\\\-?\\\\d+(?:.\\\\d+)?)$\') AS FLOAT64)) FROM UNNEST(ar) as val));`,
    ...
    '  model_main AS (',
    '    SELECT',
    `      COALESCE(mprove_array_sum(ARRAY_AGG(DISTINCT CONCAT(CONCAT(CAST(a.dim4 + 200 AS STRING), '||'), CAST(a.dim2 + 100 AS STRING)))), 0) as a_mea1`,
    '    FROM (',
    '      SELECT',
    '        (111) + 222 as dim2,',
    '        (333) + 444 as dim4',
  ...
  return ApStruct.rebuildStruct({
    dir: 'test/valid/03_mea_refs_dim/v4',
