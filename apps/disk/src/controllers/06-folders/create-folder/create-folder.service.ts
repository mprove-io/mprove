...
import type { ToDiskCreateFolderResponsePayload } from '#common/zod/to-disk/06-folders/to-disk-create-folder';
import { zToDiskCreateFolderRequest } from '#common/zod/to-disk/06-folders/to-disk-create-folder';
...
export class CreateFolderService {
  ...
  async process(request: any) {
    ...
    let { orgId, baseProject, repoId, branch, folderName, parentNodeId } =
    ...
    let isParentPathExist = await isPathExist(parentPath);
    ...
    await ensureDir(folderAbsolutePath);
    ...
    let payload: ToDiskCreateFolderResponsePayload = {
      repo: {
        orgId: orgId,
        projectId: projectId,
        repoId: repoId,
