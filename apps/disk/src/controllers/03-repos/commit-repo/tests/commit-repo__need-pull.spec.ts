...
test('1', async t => {
  let resp: ToDiskCommitRepoResponse;
  ...
  try {
    ...
    let r1_master_commitRepoRequest: ToDiskCommitRepoRequest = {
    ...
    await messageService.processRequest({ request: r1_master_saveFileRequest });
    await messageService.processRequest({
      request: r1_master_commitRepoRequest
    });
    await messageService.processRequest({ request: r1_master_pushRepoRequest });

    await messageService.processRequest({
      request: r2_master_createFileRequest
    });
