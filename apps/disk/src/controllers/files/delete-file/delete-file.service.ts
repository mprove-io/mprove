...
export class DeleteFileService {
  ...
  async process(item: {
    ...
    fileNodeId: string;
    userAlias: string;
  }): Promise<ToDiskResponseResultForOperation<'deleteFile'>> {
    ...
    let deleteFileResult = Result.pipe(
      ...
      Result.andThrough(
        (v): Result.Result<void, DiskFileIsNotExistError> =>
          v.isFileExist === false
            ? Result.fail({ code: 'DISK_FILE_IS_NOT_EXIST' })
            : Result.succeed()
      ),
      Result.andThrough(v => removePath({ path: v.filePath })),
      ...
      Result.andThrough(v =>
        v.repoId === PROD_REPO_ID
          ? commit({
              ...
              commitMessage: `Deleted file ${v.relativeFilePath}`
            })
          : Result.succeed()
      ),
      Result.andThrough(v =>
      ...
      Result.map(
        (v): ToDiskDeleteFileOutput => ({
          ...
          deletedFileNodeId: v.fileNodeId,
