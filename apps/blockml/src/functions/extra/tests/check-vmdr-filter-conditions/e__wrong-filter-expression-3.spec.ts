...
test('1', async t => {
  let errors: BmError[];
  let entitiesDashboards: FileDashboard[];
  ...
  try {
    ...
    entitiesDashboards = await readLog(fromDir, LogTypeEnum.Entities);
    if (isDefined(toDir)) {
  ...
  t.is(errors[0].title, ErTitleEnum.WRONG_FILTER_EXPRESSION);
