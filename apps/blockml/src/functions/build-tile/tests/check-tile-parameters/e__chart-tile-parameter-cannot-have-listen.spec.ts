...
test('1', async t => {
  ...
  try {
    ...
    entCharts = await readLog(fromDir, LogTypeEnum.Entities);
    if (isDefined(toDir)) {
  ...
  t.is(errors[0].title, ErTitleEnum.CHART_TILE_PARAMETER_CANNOT_HAVE_LISTEN);
