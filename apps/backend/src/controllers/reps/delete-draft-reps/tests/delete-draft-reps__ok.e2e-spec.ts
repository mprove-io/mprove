...
let testId = 'backend-delete-draft-reps__ok';
...
test('1', async t => {
  let resp2: apiToBackend.ToBackendDeleteDraftRepsResponse;
  ...
  try {
    ...
    let req2: apiToBackend.ToBackendDeleteDraftRepsRequest = {
      info: {
        name: apiToBackend.ToBackendRequestInfoNameEnum
          .ToBackendDeleteDraftReps,
        traceId: traceId,
        idempotencyKey: common.makeId()
      },
      payload: {
        ...
        branchId: common.BRANCH_MASTER,
        envId: common.PROJECT_ENV_PROD,
        repIds: [resp1.payload.rep.repId]
      }
    };

    resp2 =
      await helper.sendToBackend<apiToBackend.ToBackendDeleteDraftRepsResponse>(
        {
          httpServer: prep.httpServer,
          loginToken: prep.loginToken,
          req: req2
