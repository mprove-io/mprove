...
let testId = 'backend-save-modify-rep__ok';
...
test('1', async t => {
  let resp2: apiToBackend.ToBackendSaveModifyRepResponse;
  ...
  try {
    ...
    let req2: apiToBackend.ToBackendSaveModifyRepRequest = {
      info: {
        name: apiToBackend.ToBackendRequestInfoNameEnum.ToBackendSaveModifyRep,
        traceId: traceId,
        idempotencyKey: common.makeId()
      },
      payload: {
    ...
    resp2 =
      await helper.sendToBackend<apiToBackend.ToBackendSaveModifyRepResponse>({
        httpServer: prep.httpServer,
        loginToken: prep.loginToken,
        req: req2
      });
