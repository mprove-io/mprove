...
let testId = 'backend-mcp-get-skills__ok';
...
test('1', async t => {
  ...
  await retry(async (bail: any) => {
    ...
    assert.ok(Array.isArray(structuredContent.skillItems));
    assert.ok(structuredContent.skillItems.length > 0);
    assert.equal(typeof structuredContent.skillItems[0].name, 'string');
    assert.equal(typeof structuredContent.skillItems[0].content, 'string');
