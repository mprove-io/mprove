...
test('1', async t => {
  ...
  await retry(async () => {
    let resp: ToBackendAddProviderModelResponse;
    ...
    try {
      ...
      let req: ToBackendAddProviderModelRequest = {
        info: {
          name: ToBackendRequestInfoNameEnum.ToBackendAddProviderModel,
          traceId: traceId,
        ...
        payload: {
          projectId: projectId,
          providerId: providerId,
          model: { modelId: 'model-2', name: 'Model Two' }
        }
      };

      resp = await sendToBackend<ToBackendAddProviderModelResponse>({
