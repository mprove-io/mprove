...
import {
  ...
  ToBackendCreateTempMconfigResponse
...
test('1', async t => {
  let resp2: ToBackendCreateTempMconfigResponse;
  ...
  try {
    ...
    let req2: ToBackendCreateTempMconfigRequest = {
      info: {
        name: ToBackendRequestInfoNameEnum.ToBackendCreateTempMconfig,
        traceId: traceId,
        idempotencyKey: makeId()
      },
      payload: {
    ...
    resp2 = await sendToBackend<ToBackendCreateTempMconfigResponse>({
      httpServer: prep.httpServer,
      loginToken: prep.loginToken,
      req: req2
    });
