...
export function makeJoinsAndSetLabelsAndDescriptions(item: {
  ...
  item.models.forEach(x => {
    ...
    x.joins.forEach(j => {
      ...
      let name = j.as === x.from_as ? j.from_view : j.join_view;
      ...
      if (view) {
        ...
        j.label = j.label ? j.label : j.view.label;

        j.label_line_num = j.label_line_num ? j.label_line_num : 0;

        j.description = j.description ? j.description : j.view.description;
        ...
        let line =
          j.as === x.from_as ? j.from_view_line_num : j.join_view_line_num;
