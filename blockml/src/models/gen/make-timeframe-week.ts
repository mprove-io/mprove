  ...
  if (item.connection === api.ProjectConnectionEnum.BigQuery) {
    ...
    let dayOfWeekIndex =
      item.week_start === api.ProjectWeekStartEnum.Sunday
        ...
        : `(CASE WHEN EXTRACT(DAYOFWEEK FROM ${
            item.sql_timestamp
          }) = 1 THEN 7 ELSE ` +
    ...
    let fullWeekStartDate =
      item.week_start === api.ProjectWeekStartEnum.Sunday
        ? `CAST(CAST(TIMESTAMP_TRUNC(CAST(${
            item.sql_timestamp
          } AS TIMESTAMP), WEEK) AS DATE) AS STRING)`
        : `CAST(DATE_ADD(CAST(TIMESTAMP_TRUNC(CAST(${
            item.sql_timestamp
          } AS TIMESTAMP), WEEK) AS DATE), INTERVAL 1 DAY) AS STRING)`;

    sql = `CASE
WHEN ${dayOfYear} >= ${dayOfWeekIndex} THEN ${fullWeekStartDate}
ELSE CAST(DATE_ADD(CAST(${
      item.sql_timestamp
    } AS DATE), INTERVAL -${dayOfYear} + 1 DAY) AS STRING)
END`;
