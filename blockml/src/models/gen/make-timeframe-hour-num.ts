...
export function makeTimeframeHourNum(item: {
  ...
  if (item.connection === api.ProjectConnectionEnum.BigQuery) {
    sql =
      ...
      `MOD(-1 * EXTRACT(HOUR FROM ${item.sql_timestamp}), ${
        item.num
      }) HOUR), HOUR))`;
  } else if (item.connection === api.ProjectConnectionEnum.PostgreSQL) {
    sql = `TO_CHAR(DATE_TRUNC('hour', DATE_TRUNC('hour', ${
      item.sql_timestamp
    } - (DATE_PART('hour', ${item.sql_timestamp})::INT % ${
      item.num
    } || 'HOURS')::INTERVAL)), 'YYYY-MM-DD HH24')`;
  }
