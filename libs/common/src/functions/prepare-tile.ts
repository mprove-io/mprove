...
export function prepareTile(item: {
  ...
  if (isForDashboard === true && isDefined(tile)) {
    filePartTile.plate = {
      plate_width:
        // tile.plateWidth !== constants.TILE_DEFAULT_PLATE_WIDTH &&
        isDefined(tile.plateWidth) ? <any>tile.plateWidth : undefined,

      plate_height:
        // tile.plateHeight !== constants.TILE_DEFAULT_PLATE_HEIGHT &&
        isDefined(tile.plateHeight) ? <any>tile.plateHeight : undefined,

      plate_x:
        // tile.plateX !== constants.TILE_DEFAULT_PLATE_X &&
        isDefined(tile.plateX) ? <any>tile.plateX : undefined,

      plate_y:
        // tile.plateY !== constants.TILE_DEFAULT_PLATE_Y &&
        isDefined(tile.plateY) ? <any>tile.plateY : undefined
    };
  }
  ...
  // let keepAxis = false;
  // Object.keys(filePartTile.axis).forEach((x: any) => {
  //   if (isDefined((<any>filePartTile.axis)[x])) {
  //     keepAxis = true;
  //   }
  // });
  // if (keepAxis === false) {
  //   delete filePartTile.axis;
  // }
