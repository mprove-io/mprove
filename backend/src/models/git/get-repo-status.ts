...
export async function getRepoStatus(item: {
  ...
  let head = <nodegit.Commit>(
    await gitRepo
      .getHeadCommit()
      .catch(e =>
        helper.reThrow(e, enums.nodegitErrorsEnum.NODEGIT_REPO_GET_HEAD_COMMIT)
      )
  );

  let treeHead = <nodegit.Tree>(
    await head
      .getTree()
      .catch(e =>
        helper.reThrow(e, enums.nodegitErrorsEnum.NODEGIT_COMMIT_GET_TREE)
      )
  );

  const diffTreeToIndex = <nodegit.Diff>(
    await nodegit.Diff.treeToIndex(gitRepo, treeHead, null).catch(e =>
      helper.reThrow(e, enums.nodegitErrorsEnum.NODEGIT_DIFF_TREE_TO_INDEX)
    )
  );

  const patchesTreeToIndex = <nodegit.ConvenientPatch[]>(
    await diffTreeToIndex
      .patches()
      .catch(e =>
        helper.reThrow(e, enums.nodegitErrorsEnum.NODEGIT_DIFF_PATCHES)
      )
  );
  ...
  let itemDevRepoCatalog = <interfaces.ItemCatalog>await disk
  ...
  await gitRepo
    .fetch('origin', constantFetchOptions)
