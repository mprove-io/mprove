...
export async function commit(item: {
  ...
  let head = <nodegit.Oid>(
    await nodegit.Reference.nameToId(gitRepo, 'HEAD').catch(e =>
      helper.reThrow(e, enums.nodegitErrorsEnum.NODEGIT_REFERENCE_NAME_TO_ID)
    )
  );

  let parent = <nodegit.Commit>(
    await gitRepo
      .getCommit(head)
      .catch(e =>
