...
function checkResponseStatus(response: any) {
  if (
    ...
    response.data.info.status ===
      api.BlockmlResponseStatusEnum.BlockmlWrongRequestParams
  ) {
    throw new ServerError({
      name: enums.otherErrorsEnum.BLOCKML_RESPONSE_WRONG_REQUEST_PARAMS
    });
  } else if (
    response &&
    response.data &&
    response.data.info &&
    response.data.info.status ===
      api.BlockmlResponseStatusEnum.BlockmlInternalError
  ) {
    throw new ServerError({
      name: enums.otherErrorsEnum.BLOCKML_RESPONSE_INTERNAL
    });
  } else {
    throw new ServerError({
      name: enums.otherErrorsEnum.BLOCKML_RESPONSE_NOT_OK
    });
  }
}
