...
Use get-stores tool to understand existing HTTP API (non-SQL) connections added by User.
...
If user requested HTTP API data sources that is not presend in get-stores response - ask user to add connection first. You can guide user based on searched HTTP API docs.
